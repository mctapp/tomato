...
import { AlertCircle, Play, Pause, RefreshCw, Volume2, FileAudio, Copy } from 'lucide-react';
import { toast } from 'sonner'; // sonner가 설치되어 있지 않다면 다른 toast 라이브러리 사용

interface AudioPlayerProps {
  src: string;
  title: string;
  className?: string;
  showCopyButton?: boolean; // 복사 버튼 표시 여부를 위한 props 추가
}
...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ 
  ...
  const [isCopied, setIsCopied] = useState(false); // URL 복사 상태
  ...
  useEffect(() => {
    ...
    const handleError = (e: Event) => {
      ...
      setError("오디오 파일을 로드할 수 없습니다. 지원되지 않는 형식이거나 접근 권한이 없습니다.");
      setIsLoading(false);
    };
  ...
  const handleCopyUrl = () => {
    ...
    navigator.clipboard.writeText(src)
      .then(() => {
        setIsCopied(true);
        if (toast) {
          toast.success("오디오 URL이 클립보드에 복사되었습니다");
        }
      })
      .catch(err => {
      ...
      <div className="flex items-center justify-between">
        ...
        {showCopyButton && (
          <Button
            variant="ghost"
            size="icon"
            className="h-7 w-7 rounded-full hover:bg-blue-50 hover:text-blue-500 text-gray-400"
